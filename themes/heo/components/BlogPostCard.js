  ...
  return (
        <div className={` ${CONFIG.POST_LIST_COVER_HOVER_ENLARGE ? ' hover:scale-110 transition-all duration-150' : ''}`} >
            ...
            <div
                ...
                className={'border bg-white dark:bg-[#1e1e1e] flex mb-4 flex-col h-[23rem] md:h-52 md:flex-row 2xl:h-96 2xl:flex-col group w-full dark:border-gray-900 hover:border-gray-900  dark:hover:border-green-400 duration-300 transition-colors justify-between overflow-hidden rounded-xl'}>
                ...
                <div className={'flex p-6 2xl:p-4 flex-col justify-between h-48 md:h-full 2xl:h-48 w-full md:w-7/12 2xl:w-full'}>
                    <div>
                        {/* 分类 */}
                        {post?.category && <div className={`flex mb-1 items-center ${showPreview ? 'justify-center' : 'justify-start'} hidden md:block flex-wrap dark:text-gray-900 text-gray-900 `}>
                            <Link passHref href={`/category/${post.category}`}
                                className="cursor-pointer text-xs font-normal menu-link hover:text-green-400 dark:hover:text-green-400  dark:text-gray-900 transform">
                                {post.category}
                            </Link>
                        </div>}
                    ...
                    {(!showPreview || showSummary) && (
                        <p className="line-clamp-2 replace my-3 2xl:my-1 text-gray-900  dark:text-gray-300 text-sm font-light leading-tight">
                            {post.summary}
                        </p>
                    )}
